 ...
 * https://github.com/vaporyco/go-vapory/blob/develop/crypto/ecies/ecies.go#L134
...
ECIES.prototype.parseAuth = function (data) {
  ...
  var he = vapUtil.sha3(ephemeral).toString('hex')
...
ECIES.prototype.createAuth = function () {
  ...
  var he = vapUtil.sha3(ephemeralPubKey)
...
ECIES.prototype.parseHeader = function (data) {
  ...
  var size = this._bodySize = vapUtil.bufferToInt(header.slice(0, 3))
...
ECIES.prototype.createHeader = function (size) {
  // parse header
  size = vapUtil.pad(vapUtil.intToBuffer(size), 3)
...
ECIES.prototype.setupFrame = function (remoteData) {
  ...
  var hNonce = vapUtil.sha3(nonceMaterial)
  var sharedSecret = vapUtil.sha3(Buffer.concat([this.ephemeralSecret, hNonce]))
  this.token = vapUtil.sha3(sharedSecret)
  this.aesSecret = vapUtil.sha3(Buffer.concat([this.ephemeralSecret, sharedSecret]))
  this.macSecret = this.egressMac.secret = this.ingressMac.secret = vapUtil.sha3(Buffer.concat([this.ephemeralSecret, this.aesSecret]))
  var IV = new Buffer(16)
  IV.fill(0)
